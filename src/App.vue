  ...
  #app
    ...
    #appMain
      b-navbar.shadow.mb-1(v-if="user.id.length === 0 " toggleable='lg')
        b-container
          b-navbar-brand(to='/')
            img.logo(src='../public/img/puipui.png')
            //- img.logo2(src='../public/img/logo2.png')
            span.logotitle &nbsp;&nbsp;TEST PAGE
        ...
        b-collapse#nav-collapse(is-nav)
          b-navbar-nav.ml-auto.font-weight-bold
            ...
            b-nav-item(v-if="user.id.length === 0 | user.id.length> 0" to='/form') 表單測試
            facebook-login.button(appid='420905468863679', @login='onLogin', @logout='onLogout', @get-initial-status='getUserData', @sdk-loaded='sdkLoaded')
            .information(v-if='isConnected')
              h1 My Facebook Information
              .well
                .list-item
                  img(:src='picture')
                .list-item
